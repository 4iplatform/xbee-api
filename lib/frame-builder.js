...
frame_builder[C.FRAME_TYPE.BLUETOOTH_UNLOCK] = function(frame, builder) {
  builder.appendUInt8(frame.type);
  builder.appendUInt8(frame.id);
  builder.appendString(frame.command, 'hex');
  appendData(frame.commandParameter, builder);
};

frame_builder[C.FRAME_TYPE.USER_DATA_RELAY] = function(frame, builder) {
  builder.appendUInt8(frame.type);
  builder.appendUInt8(frame.id);
